...
! yearnum = 1
! nyears = 2
! CALL getenv("PBS_VNODENUM", ellc)
! read(ellc,*) ell
! ell = ell + 30 + 24*2
! ellp = ell
 
! print *,yearnum, ell, ellp, nyears
 if (1==2) then
 call cpu_time(tstart)
 call analyzethis(yearnum, ell, ellp, nyears) !(1,31,30,1)
 call cpu_time(tfin)
 ...
 call system('rm /scratch/shravan/MDI/processed/'//yearc//'_'//ellc)
...
!  ell =120
!  ord = 72 * 24 * 60
!  call readfits('/scratch/data/MDI/mdi.vw_V_sht_gf_72d.1216.120.fits', aout, ord,ell+1,1)
!  amat(1:ord,:) = (aout(1:2*ord:2,:) - (0.d0, 1.d0) * aout(2:2*ord:2,:))*60.

!  stop

 ! ell = 120
 !do ord=-120,0
 ! print *,leakage(ell)%ems(ord),leakage(ell)%ems(-ord)
 !enddo
 !stop
